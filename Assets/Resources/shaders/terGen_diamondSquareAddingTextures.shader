Shader "Custom/TerrainCreation/DiamondSquareTextureAdding"
{
	Properties
	{
	...
	SubShader
	{
		...
		ZTest Always Cull Off ZWrite Off Fog { Mode Off } //Rendering settings
			...
			fixed4 frag(v2f i) : Color{
				...
				float height1 = decodeHeight(tex2D(_Texture1, pos));
				float height2 = decodeHeight(tex2D(_Texture2, pos));
				float revamped2 = height2 / 32 - 0.015;

				float4 finalValue = encodeHeight(height1 + revamped2/8);
