Shader "Custom/Sandbox/Filling/MMGeometryAlphaRenderer" {
	Properties{
...
#include "mf_common.txt"
			...
			struct g2f {
				float4 pos : SV_POSITION;
				float vCoord : ANY_VCOORD;
				uint id : ANY_ID;
				float tParam : ANY_TPARAM;
			};

			g2f make_g2f(float4 pos, float vCoord, uint id, float tParam) {
				g2f g;
				g.pos = pos;
				g.vCoord= vCoord;
				g.id = id;
...
#define SEGMENT_COUNT 2
			...
			void geom(point v2g input[1], uint pid : SV_PrimitiveID, inout TriangleStream<g2f> outStream)
			{
				...
				float segmentLength =  paramPack.hatchLength*2  / fFac;
					...
					for (int i = 0; i < SEGMENT_COUNT; i++) {
						...
						float2 strokeD = (float2(sin(hatchAngle), cos(hatchAngle))).yx*  t   * segmentLength;
					...
					for (int i = 0; i < SEGMENT_COUNT; i++) {
						float t = ((float)i) / (SEGMENT_COUNT - 1);
						outStream.Append(make_g2f(float4(newPoints[i * 2 + 1], 1, 1), 0, seedId, t));
						outStream.Append(make_g2f(float4(newPoints[i * 2 + 0], 1, 1), 1, seedId, t));
					}
		    ...
		    mf_MRTFragmentOutput frag(g2f input) : SV_Target
			{
				...
				float2 hatchCoords = float2(input.tParam, input.vCoord);
				artisticColor = tex2D(_StrokeTex, hatchCoords);


				//uint tier =  input.id>> 30;
				//if (tier == 0) {
				//	artisticColor.r += 1;
				//}
				//else if (tier == 1) {
