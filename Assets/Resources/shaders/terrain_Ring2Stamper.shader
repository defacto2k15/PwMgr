Shader "Custom/Terrain/Ring2Stamper"
	{
	Properties 
	{ 
...
// Upgrade NOTE: excluded shader from DX11; has structs without semantics (struct v2f members uv_HeightTex)
			...
			fixed4 frag (v2f i) : Color{ 
				...
				TextureLayerOutput output = ring2_surf( float3( pos.x, 0, pos.y), i.uv.xy);

				float xInt = fmod(pos.x, 45.0) / 45.0;
				float yInt = fmod(pos.y, 45.0) / 45.0;
