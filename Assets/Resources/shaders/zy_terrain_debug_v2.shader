Shader "Custom/Terrain/Terrain_Debug_V2"
{
	Properties 
	{
		...
		_OverlaySelection("OverlaySelection", Range(0,1))=0
	...
	SubShader 
	{
		Cull Off ZWrite On  //Rendering settings
			...
			float2 getHeightmapTextureValues( float2 uvPos){ //position in object
				...
				float heightmapValue1 = decodeHeight(tex2Dlod(_HeightmapTex1, float4( ring1Texture,0,0)));
			...
			void surf (in Input IN, inout SurfaceOutputStandard o)  
			{
				...
				if (_OverlaySelection > 0.5) {
					outColor = tex2D(_OverlayTex, uv.yx)*_DebugScalar;
				}
