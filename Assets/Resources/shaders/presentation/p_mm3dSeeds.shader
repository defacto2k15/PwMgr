Shader "Custom/Presentation/MM3DSeeds" {
	Properties{
		...
		SubShader
	{
		...
		Pass
		{
			...
			MRTFragmentOutput	frag(v2f input) : SV_Target
			{
				...
				float3 worldSpaceSeedPosition = transportFromGridSpace(normalVector, alignmentVector, spec.position );
				if (spec.isActive) {
				...
				color0.xy += debugSurfaceColor3d(worldSpacePos) / 3.0;
