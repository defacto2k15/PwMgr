Shader "Custom/Presentation/Szecsi" {
	Properties{
...
#define DRAW_DOTS (1)
			...
			fixed4 frag (v2f input) : SV_Target
			{
				...
				for (int i = 0; i < 16; i++) { // for each seed
...
#if DRAW_DOTS
					float dotPaintingRadius = 0.01 *pow(2, m);
					...
					float hatchStrokeScale = pow(2,1-m)/(_StrokeSize*_FSeedDensity/1000);
					...
					//float2 texUv = uv;
					float2x2 rotMatrix = createRotationMatrix(_Rotation);
					...
					float4 texColor = tex2D(_HatchTex, float2(texUv));
					alpha = alpha * texColor.a;
#if DRAW_HATCHES
					color = (1 - alpha)*color + alpha * texColor;
#endif
