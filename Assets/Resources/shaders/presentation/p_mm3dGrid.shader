Shader "Custom/Presentation/MM3DGrid" {
	Properties{
		...
		SubShader
	{
		...
		Pass
		{
			...
			MRTFragmentOutput	frag(v2f input) : SV_Target
			{
				...
				ClosestSeedSpecification spec = retriveClosestSeed(GSSpacePos);
				spec.position = spec.position.zyx;
				float3 worldSpaceSeedPosition = transportFromGridSpace(normalVector, alignmentVector, memberwiseMultiplyF3(spec.position, 1/paramPack.gridSpaceMultiplier)/seedSpaceMultiplier   );
				if (spec.isActive) {
					sgmColor = calculateSgmColor2(worldSpacePos, worldSpaceSeedPosition, normalVector, strokeAngle, paramPack, screenCoords);
				}
				...
				color0.xy += debugSurfaceColor3d(GSSpacePos) / 3.0;

				if (length(sgmColor.xy - screenCoords) < 3) {
					color0.xy = 1;
				}
